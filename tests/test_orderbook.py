...
class TestOrderBook(unittest.TestCase):
    ...
    def test_place_bid_order(self):
        order_params = OrderParams(OrderSide.BID, Decimal("100.5"), 10, OrderType.GTC)
    ...
    def test_place_ask_order(self):
        order_params = OrderParams(OrderSide.ASK, Decimal("101.0"), 5, OrderType.GTC)
        result = self.order_book.process_one(order_params)
        self.assertTrue(result.success)
    ...
    def test_execute_market_order(self):
        bid_params = OrderParams(OrderSide.BID, Decimal("100.5"), 10, OrderType.GTC)
        ask_params = OrderParams(OrderSide.ASK, Decimal("100.5"), 10, OrderType.GTC)
    ...
    def test_spread_calculation(self):
        self.order_book.process_one(OrderParams(OrderSide.BID, Decimal("100.0"), 10, OrderType.GTC))
        self.order_book.process_one(OrderParams(OrderSide.ASK, Decimal("102.0"), 10, OrderType.GTC))
